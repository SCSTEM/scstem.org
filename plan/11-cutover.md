...
- Confirm preview deployments still noindexed (`_headers` covers `*.pages.dev` + every `*.scstem.org` subdomain; per D25 those are the only noindex rules left — `/team/*`, `/image/*`, `/video/*` are gone deliberately).
